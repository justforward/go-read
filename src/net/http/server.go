...
// completes.
//将缓冲区的数据直接发送给client 不需要等待，有数据就发送
...
type Hijacker interface {
	...
	// 在调用Hijack之后，HTTP服务器库将不会对连接执行任何其他操作
...
// 在开启的 goroutine中conn的serve()会进行路由匹配找到路由处理函数然后调用处理函数
...
// handlerFunc 实现了 handler的方法
type HandlerFunc func(ResponseWriter, *Request)

// ServeHTTP calls f(w, r).
func (f HandlerFunc) ServeHTTP(w ResponseWriter, r *Request) {
	f(w, r) // 调用的f 是注册进入的func
}
...
//只不过ServeMux的ServeHTTP方法不是用来处理具体的request和构建response，
//而是用来通过路由查找对应的路由处理器Handler对象，
//再去调用路由处理器的ServeHTTP 方法去处理request和构建reponse。
func (mux *ServeMux) ServeHTTP(w ResponseWriter, r *Request) {
	if r.RequestURI == "*" { // 结构体中包含了handler 但是每个
	...
	h, _ := mux.Handler(r) // 根据request得到 注册进入的函数
	h.ServeHTTP(w, r) // 调用这个方法的serverHTTP
}
...
// serverHandler 会代理server对象
func (sh serverHandler) ServeHTTP(rw ResponseWriter, req *Request) {
	// sh.srv.Handler就是我们最初在http.ListenAndServe()中传入的Handler参数
	handler := sh.srv.Handler
	// 如果handler 为nil  使用默认的 DefaultServeMux
	if handler == nil {
		// handler
...
/*
这里隐去了一些细节，以便了解Serve方法的主要逻辑。
首先创建一个上下文对象，然后调用Listener的Accept()接收监听到的网络连接；
一旦有新的连接建立，则调用Server的newConn()创建新的连接对象，
并将连接的状态标志为StateNew，然后开启一个goroutine处理连接请求
 */
...
//net http 核心包 这里的handler是一个接口
func ListenAndServe(addr string, handler Handler) error {
	//首先构建一个server gin里面可以对这个sever进行一些定制化操作 然后进行执行listenAndServe()
