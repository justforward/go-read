...
//主入口：根据传入的request  由此选择一个合适的长连接来发送该request并返回response
...
func (t *Transport) getConn(treq *transportRequest, cm connectMethod) (pc *persistConn, err error) {
	...
	// Queue for idle connection. 找到空闲的连接
	if delivered := t.queueForIdleConn(w); delivered {
		...
		// 将request cancel设置为某个非空函数，这样我们就可以检测到从现在到进入roundTrip时它是否被清除了
...
type connectMethod struct {
	//忽略这个变量，意思是那个位置本应赋给某个值，但是咱们不需要这个值。所以就把该值赋给下划线，意思是丢掉不要。
	_            incomparable
	//代理的url
